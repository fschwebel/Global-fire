...
> **Canon note** (binding, from [`../PLAN.md`](../PLAN.md) §2): campaign 2026–2035 · one persistent 48×32 map, 1 tile = 1 ha · 2 Hz fixed ticks, square grid, Moore-8, double-buffered, seeded PRNG · no game-over/stars/grades; internal Stewardship Score only · deterministic firefighter deaths · warning-time evacuation mortality · unlock-year tools granted free · every number below is a **v0 value to playtest**, living in `balance.ts` — tuning never touches sim code.
...
**Per-year loop (one level ≈ 3–5 minutes):**

1. **Briefing (~15 s):** year card, season forecast (dryness, wind regime, expected ignitions), one sourced climate fact, unlock panel if any.
2. **Prep phase (untimed; from 2027):** spend budget — buy/repair trucks, place towers, position units. **2026 skips this phase entirely** — the tutorial fire is called in within 5 seconds of the season starting.
...
4. **Debrief (~20 s):** losses tally one counter at a time; quiet narrative line; budget for next year computed; next unlock teased. No stars, no grades.

**Campaign loop (10 levels, 2026–2035):** climate parameters ratchet every year (§7); one new mechanic *or* one new loss counter per year, so the player always learns exactly one thing while conditions worsen underneath them. 2033–2035 deliberately add no new tools — the toolkit is complete and increasingly insufficient. That beat *is* the message.
...
| `moistureFactor` | `0.35 + 1.05 × dryness`, dryness ∈ [0,1] | Dryness is *the* climate lever, set per year (§7.1) |
| `windFactor` | `clamp(1 + windStr × cos θ, 0.15, 1 + windStr)`; θ = angle wind vs. B→C; `windStr` 0.2 (2026) → 1.4 (2035) | Downwind up to ~2.4×; upwind floor keeps fronts alive |
...
Spotting is why firebreaks and rivers stop being absolute safety in later years — a direct, legible "the rules you learned no longer hold" climate metaphor. Disabled before 2030.

### 2.6 Ignition sources

Per year: an authored script (`ignitionsScripted`: fixed ticks + regions) plus a background rate (`P_RANDOM_IGNITION` per tick anywhere flammable; flavour rotates: lightning, powerline, negligence). 2026: 1 scripted, no random. 2035: 4 scripted + 0.004/tick.
...
- **One curated procedural seed for the entire campaign.** Years are ignition/weather scripts on the same map. Burn scars, regrowth, and the 2031 WUI development all accumulate on it — the map is the campaign's diary (see ux doc §1.6).
...
| Firebreak (bare earth) | ~no (0.05) | Pre-authored from the start; player-cut from 2031 | 1 % |
...
3. **Villages: 2–4 clusters of 6–20 house tiles** in grass/sparse clearings with a 1–2 tile buffer from dense forest. Each house tile: **4–10 occupants** → valley population ~250–600. In **2031** the script adds new house tiles *inside* the forest edge ("development pushed into the wildland-urban interface" — a true climate-adjacent fact for that year's briefing).
...
### 3.4 Scars and regrowth (year boundaries)

Burnt cells regrow: ash → grassland (1 yr) → sparse (3 yr) → dense (6–10 yr). Cells burnt at intensity ≥ 8 roll **25 % permanent conversion** to grassland/shrub (habitat and fuel both reduced — a scarred map is genuinely a bit safer, as in reality, which keeps bad years from snowballing).
...
- **Cooldowns** pace the real-time layer (bomber sorties, evac orders). Action points rejected (turn-based artifact); cooldowns-only rejected (no strategic layer, no year-over-year consequence).
- **Budget:** `budget(y) = 80 + 12·t + 0.25 × unspent(y−1) + perfBonus(y−1)`, where `t = year − 2026` and `perfBonus = round(score/50)` (0–20, from the internal score §6.2). Clamped to a floor `minViable(y) = 90 + 6·t` that always affords the minimum effective loadout — the campaign is never mathematically lost.
- Purchases persist across years (trucks survive unless destroyed; towers are permanent) — losing a truck hurts next year too.
- **Unlock-year grants are free** ("reassigned to your sector"): 2 trucks (2026), the evacuation capability (2027), water bomber #1 (2028), tower #1 (2029), fire crew #1 (2031). Budget buys extras, repairs, replacements, and additional placements.
...
**Firefighter danger rule (active from 2030, with the metric reveal):** if a truck's tile is BURNING, or it is adjacent to ≥ 3 cells of intensity ≥ 7 for 3 consecutive ticks, it broadcasts a radio warning ("Crew 2 requesting pull-out") and auto-retreats to the nearest safe tile after the 3-tick grace. It is destroyed — 4 firefighters lost — **only if no safe path exists**. Deterministic, never dice: every death traces to a player choice (held position, escape route allowed to close). Before 2030 the destruction clause is disabled; pre-spotting fire intensities cannot produce entrapment.

### 4.3 Evacuation order (2027)
...
**Preparedness (hidden scalar, 0.6–1.0, starts 1.0):** a village evacuated but never threatened (no fire within 10 tiles all season) → −0.1 ("evacuation fatigue"; future evacuations run slower). Recovers +0.05 per year. This replaces any score penalty for over-evacuating — the cost of crying wolf is operational, not moral bookkeeping.

### 4.4 Water bomber (2028; UI never says "Canadair" — trademark)

| Stat | v0 |
|---|---|
| Fleet | #1 free in 2028; #2 purchasable from 2032 at 150 |
...
### 4.5 Watch tower (2029)

| Stat | v0 |
|---|---|
| Cost | 30, prep phase only, permanent; #1 free in 2029; cap 4 |
| Radius | 8 tiles |
...
### 4.6 Fire crew & controlled burns (2031)

| Stat | v0 |
|---|---|
| Fire crew (bulldozer) | 25; #1 free in 2031; cuts firebreak 1 tile / 3 ticks, lines up to 8 tiles; crew of 2, subject to the danger rule |
...
Controlled burns consume fuel ahead of a front — high skill ceiling, real-world authentic (prescribed burning / Indigenous stewardship practice), and self-balancing: late-campaign weather makes them genuinely dangerous. Pre-authored firebreaks exist on the map from 2026; *cutting new ones* requires the crew.

---

## 5. Unlock & reveal schedule

Canonical table in [`../PLAN.md`](../PLAN.md) §2.2; teaching rationale in [`progression.md`](progression.md) §3. One new thing per year, never a tool tutorial and a dread reveal in the same moment (ux doc §2.2).
...
The season timer **is** the autumn rains: at `seasonLen(y)` ticks they arrive and end every fire (a somber fast-forward montage shows anything still burning take what it can first). `seasonLen` grows every year — the rains come later, which is both the difficulty curve and the visible climate signal on the HUD's season bar. If all fires are out and no scripted ignitions remain, the year can end early. **There is no fail screen and no replay prompt.** One exception: if every village is lost, the campaign cuts to the early ending "The Year the Line Broke" (ux doc §1.7).

Mid-season **relief rains** (all fires −3 intensity, global wet 20 ticks) are guaranteed 2026–2028, 50 % likely 2029–2032, and **never occur from 2033** — the 2033 briefing says so, pointedly.
...
Uses: `perfBonus` for next year's budget (§4.1) and selection of ending narrative variants (progression doc §5). Metrics not yet revealed contribute 0 until their reveal year (scenarios guarantee they are 0 anyway — PLAN §2.2). The cumulative score is never surfaced as a number; the player sees only counters, comparisons, and consequences.
...
| Budget | perfBonus shrinks after bad years | `minViable` floor guarantees a fieldable force |
| Rubber-band (hidden) | After a catastrophic year (score < 250), next spike's modifiers damp 15 % | Invisible; fiction: "a mild La Niña year" |
| Preparedness | §4.3 | Floor 0.6, recovers yearly |
...
Base curve; spike-year overrides stack on top (progression doc §2.2).

| Year | dryness | windStr | Ignitions (scripted + random/tick) | Season (ticks) | Relief rain | Notes |
|---|---|---|---|---|---|---|
| 2026 | 0.30 | 0.2 | 1 + 0 | 300 | ✔ | Tutorial; scripted wind shift |
| 2027 | 0.38 | 0.3 | 1 + 0 | 300 | ✔ | Hamlet threat → evac lesson |
| 2028 | 0.44 | 0.4 | 2 + 0 | 320 | ✔ | + Long Drought overrides |
| 2029 | 0.50 | 0.5 | 2 + 0.001 | 320 | 50 % | Undetected-ignition lesson |
| 2030 | 0.56 | 0.7 | 2 + 0.002 | 340 | 50 % | + Heat Dome; spotting begins |
| 2031 | 0.62 | 0.8 | 3 + 0.002 | 360 | 50 % | WUI development appears |
| 2032 | 0.68 | 0.9 | 3 + 0.003 | 380 | 50 % | + Early Season; night windows |
| 2033 | 0.74 | 1.1 | 3 + 0.003 | 400 | ✘ | Relief rains gone for good |
| 2034 | 0.79 | 1.2 | 4 + 0.004 | 400 | ✘ | + The Interface (ember storm, convoy) |
| 2035 | 0.85 | 1.4 | 4 + 0.004 | 420 | ✘ | + The Firestorm (pyroCb, wind reversal) |
...
2. From 2030 on, the player must lose *something* every year — the design goal is triage, not perfection.
